...
def BRadius(self):
    self.setStyleSheet("QPushButton {border-radius: 60px}")
...
class Ui_Game(object):
    def setupUi(self, Game):
        ...
        def main_menu():
            self.bg_mm = QLabel(parent=self.centralwidget)
            ...
            self.bg_mmef = QLabel(parent=self.centralwidget)
            ...
            self.BiTi = QLabel(parent=self.centralwidget)
            self.BiTi.setGeometry(607, 124, 734, 422)
            self.BiTi.setPixmap(QPixmap("design/mm/BiTi.png"))

            self.quit = PushButton("design/mm/start.png", "design/mm/start_selected.png", 760, 121, self.centralwidget)
            HCursor(self.quit)
            BRadius(self.quit)
            self.quit.setGeometry(580, 618, 760, 121)

            #из главного в меню выбора уровня
            # self.quit.clicked.connect(lambda: lvl())

            self.skins = PushButton("design/mm/skins.png", "design/mm/skins_selected.png", 360, 121, self.centralwidget)
            HCursor(self.skins)
            BRadius(self.skins)
            self.skins.setGeometry(580, 790, 360, 121)

            self.quit = PushButton("design/mm/quit.png", "design/mm/quit_selected.png", 360, 121, self.centralwidget)
            HCursor(self.quit)
            BRadius(self.quit)
            ...
            self.quit = PushButton("design/mm/settings.png", "design/mm/settings_selected.png", 121, 121, self.centralwidget)
            HCursor(self.quit)
            BRadius(self.quit)
            self.quit.setGeometry(1739, 909, 121, 121)
        ...
        def slcm():
            ...
            Icon(self.music_field_center_slcm, "design/slcm/music_center_field.png")
            self.music_field_center_slcm.setIconSize(QSize(1026, 195))

            self.goto = QPushButton(parent=self.centralwidget)
            self.goto.setGeometry(1109, 915, 760, 121)
            HCursor(self.goto)
            BRadius(self.goto)
            Icon(self.goto, "design/goto.png")
            self.goto.setIconSize(QSize(760, 121))
