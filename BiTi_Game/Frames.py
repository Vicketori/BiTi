...
def sk_f(self):
    ...
    self.bg_sk = LabelPng("design/sm/bg.png", 0, 0, 1920, 1080, self.sk)
...
def sm_f(self):
    ...
    self.record_lbl_sm = LabelText(font2, 300, 100, 200, 70, 21, 59, 71, "Рекорд", self.sm)
    self.record_combo_lbl_sm = LabelText(font2, 100, 250, 300, 70, 21, 59, 71, "Комбо:", self.sm)
    self.record_score_lbl_sm = LabelText(font2, 100, 500, 300, 70, 21, 59, 71, "Счёт:", self.sm)
    self.record_accuracy_lbl_sm = LabelText(font2, 100, 750, 300, 70, 21, 59, 71, "Точность:", self.sm)
    self.record_combo_sm = LabelText(font2, 270, 250, 100, 70, 21, 59, 71, self.record_combo, self.sm)
    self.record_score_sm = LabelText(font2, 230, 500, 300, 70, 21, 59, 71, self.record_score, self.sm)
    self.record_accuracy_sm = LabelText(font2, 340, 750, 100, 70, 21, 59, 71, self.record_accuracy, self.sm)
    ...
    self.song = LabelText(font2, 920, 170, 350, 100, 241, 188, 65, self.list_song[self.selected_level], self.sm)
    self.singer = LabelText(font3, 940, 250, 300, 50, 241, 188, 65, self.list_singer[self.selected_level], self.sm)
...
def lvl_f(self):
    ...
    if self.accuracy == 100:
        self.accuracy_tx = LabelText(font2, 70, 970, 81, 71, 79, 246, 247, self.accuracy, self.lvl)
        self.accuracy_pct = LabelText(font2, 160, 970, 51, 71, 79, 246, 247, "%", self.lvl)
    else:
        self.accuracy_tx = LabelText(font2, 80, 970, 81, 71, 79, 246, 247, self.accuracy, self.lvl)
        self.accuracy_pct = LabelText(font2, 150, 970, 51, 71, 79, 246, 247, "%", self.lvl)

    self.score_tx = LabelText(font2, 1630, 15, 271, 71, 79, 246, 247, self.score, self.lvl)
    self.combo_tx = LabelText(font2, 1790, 80, 101, 61, 79, 246, 247, self.combo, self.lvl)
    ...
    self.bg_will_be_later = LabelPng("design/sm/bg.png", 0, 0, 0, 0, self.lvl_3)
    ...
    self.lc_6 = LabelPng("design/skins/" + skin + "/circle.png", 0, 0, 0, 0, self.lvl_2)
    self.lc_7 = LabelPng("design/skins/" + skin + "/circle.png", 0, 0, 0, 0, self.lvl_2)
    self.lc_8 = LabelPng("design/skins/" + skin + "/circle.png", 0, 0, 0, 0, self.lvl_2)
    self.lc_9 = LabelPng("design/skins/" + skin + "/circle.png", 0, 0, 0, 0, self.lvl_2)
    self.lc_0 = LabelPng("design/skins/" + skin + "/circle.png", 0, 0, 0, 0, self.lvl_2)
...
def win_f(self):
    ...
    self.win_lbl_win = LabelText(font1, 1170, 190, 600, 100, 21, 59, 71, "Вы выйграли!", self.win)
    self.score_field_sm_w = LabelPng("design/win/score_field.png", 35, 40, 700, 1000, self.win)
    self.combo_lbl_win = LabelText(font2, 100, 250, 300, 70, 21, 59, 71, "Комбо:", self.win)
    self.score_lbl_win = LabelText(font2, 100, 500, 300, 70, 21, 59, 71, "Счёт:", self.win)
    self.accuracy_lbl_win = LabelText(font2, 100, 750, 300, 70, 21, 59, 71, "Точность:", self.win)
    self.combo_tx_win = LabelText(font2, 270, 250, 100, 70, 21, 59, 71, self.combo_win, self.win)
    self.score_tx_win = LabelText(font2, 230, 500, 300, 70, 21, 59, 71, self.score_win, self.win)
    self.accuracy_tx_win = LabelText(font2, 340, 750, 100, 70, 21, 59, 71, self.accuracy_win, self.win)
...
def lose_f(self):
    ...
    self.lose_lbl_lose = LabelText(font1, 785, 105, 600, 300, 21, 59, 71, "Вы проиграли", self.lose)
